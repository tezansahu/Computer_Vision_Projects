...
# We will assume a "+1/-0.25" marking scheme, with 0 for an unattempted question
...
for (q, i) in enumerate(np.arange(0, len(questionCnts), 5)):
    ...
    for (j, c) in enumerate(cnts):
        ...
        # To deal with a situation where none of the options have been marked, we need the max no. of non-zero pixels in a bubble to be greater than a threshold
        bubble_threshold = 400
        if bubbled[0] < bubble_threshold:
            bubbled = (0, None) 
...
# grab the test taker
score = correct - 0.25*(len(ANSWER_KEY) - correct - unattempted)
print("[INFO] score: {:.2f}".format(score))
cv2.putText(paper, "Score: {:.2f}".format(score), (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
