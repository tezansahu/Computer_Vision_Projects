...
ap.add_argument("-w", "--width", type=float, required=True, help="width of the left-most object in the image")
ap.add_argument("-u", "--units", type=str, required=True, help="units in which the width is mentioned (in, ft, cm, mm, m)")
ap.add_argument("-p", "--precision", type=int, default=2, help="precision of measurement")
...
for c in cnts:
    # boolean to indicate if the contour is that of reference object or not
    ref = False
    ...
    cv2.putText(orig, "{:.{x}f}{s}".format(dimB, s=args["units"], x=args["precision"]), (int(tltrX - 15), int(tltrY - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    cv2.putText(orig, "{:.{x}f}{s}".format(dimA, s=args["units"], x=args["precision"]), (int(trbrX + 10), int(trbrY)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

    if ref:
        cv2.putText(orig, "Reference", (int(tltrX - 25), int(tltrY - 30)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
